  ...
  const handleChangeOwner = async (e: DropdownChangeEvent) => {
    ...
    try {
      ...
      ownerContracts.data.data.map(async (contract: any) => {
        ...
        const [docsExps, docsEvents, docsDebts, padiCurMonth, hasDebtsResponse] = await Promise.all([
          docsExpss,
          docsEventss,
          docsDebtss,
          paidCurrentMonth,
        ...
        setContractRows((prev) => [
          ...prev,
          {
            ...
            eventualityDetails: docsEvents.data.data.map((d) => ({
              ...d,
              ContractId: contract.id,
              description:
                d.description +
                ...
                {contractRows.map((contract, l) => (
                  <div key={l} className='bg-gray-50 shadow-sm border border-gray-300 dark:border-slate-700 dark:bg-slate-900 p-2'>
                    ...
                    >
                      ...
                      )} - ${formatDateDDMMYYYY(contract.contract.endDate)}`}
                    </h2>
        ...
        <Box className='!shadow-none rounded-lg !p-2 !border-none  border-gray-200 dark:!from-gray-700 dark:!to-gray-800 dark:!bg-gradient-to-tr mt-3'>
          <div id='pdf-download' className='flex gap-x-2 '>
            {[1, 2].map((pdf, index) => (
              <div
                key={index}
                className='flex  justify-between flex-col border border-gray-200 dark:border-slate-600 p-1  min-h-[750px] w-[500px]'
              >
