          ...
          <ul className='hidden sm:flex items-center gap-x-2 text-white'>
            {menuItems.map((item, index) => (
              item.to !== null ? (
                <NavLink to={item.to}
                  key={index}
                  className={({ isActive, isPending }) => `relative items-center  justify-center flex  h-[70px] text-brand2 dark:text-slate-400  dark:hover:text-brand hover:text-brand group p-1 ${isActive ? "underline !text-brand rounded-md   " : isPending ? "pending" : ""}`}
                >
                  {item.title}
                </NavLink>
              ) : (
